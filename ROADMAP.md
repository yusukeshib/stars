...
| 1' | **Zodiacal light + airglow + interstellar dust** — extend the skyglow pass with the remaining diffuse-sky components: Leinert ZL table (ecliptic coords, sun-relative longitude), broadly-isotropic airglow, Schlegel-Finkbeiner-Davis dust extinction map modulating ISL | Leinert et al. 1998 §5 (ZL) and §7 (airglow); Schlegel, Finkbeiner & Davis 1998, ApJ 500, 525 (dust) | ⬜ |
| 1' | **Per-fragment rod/cone tone reproduction** — Pattanaik 1998 multiscale model with V'(λ)-weighted scotopic chroma; should derive the empirical `KEY_SCOTOPIC` in `tonemap.wgsl` analytically | Pattanaik et al. 1998, SIGGRAPH '98; Durand & Dorsey 2002 (bilateral local-adaptation refinement) | ⬜ |
| 1' | **Catalogue colour pipeline upgrade** — B−V → T_eff → blackbody spectrum → CIE 1931 XYZ → sRGB, replacing the current piecewise-polynomial fit so the photopic input to the mesopic blend is physically calibrated | Ballesteros, F. J. 2012, EPL 97, 34008 | ⬜ |
