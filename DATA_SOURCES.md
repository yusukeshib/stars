...
- generated top-50 bright-star labels and bright-star-weighted constellation
  label anchors in `crates/catalog/build.rs`, exposed as catalog-owned neutral
  records and adapted by `crates/scene`;
...
- `crates/catalog/build.rs` (bakes the `M1`..`M110` neutral label-position table);
...
- `crates/catalog/build.rs` (bakes the NGC / IC neutral label-position table);
...
- `crates/catalog/build.rs` for embedded catalog support and generated catalog
  label metadata (`label_data.rs` in Cargo `OUT_DIR`);
- `crates/renderer/build.rs` only for renderer-owned compact constellation
